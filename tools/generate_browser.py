...
<body>
  ...
  <script>
    ...
    function syncHashForListView() {
      setHashState({ view: 'list', filter: activeFilter !== 'All' ? activeFilter : null });
    }
    ...
    function applyRouteFromHash() {
      ...
      activeFilter = typeFilters.includes(filter) ? filter : 'All';
    ...
    function filterObjects() {
      ...
      return allObjects.filter(object => object.filterType === activeFilter);
    }
      ...
      appRoot.innerHTML = `
        <div class="view-shell">
          ...
          <div class="filter-row">
            ${typeFilters.map(filter => `<button class="filter-button ${filter === activeFilter ? 'active' : ''}" data-filter="${filter}">${filter === 'All' ? 'All' : escapeHtml(formatTypeLabel(filter))}</button>`).join('')}
          </div>
          <div class="view-title">
            <span>${filtered.length} objects</span>
            <span>Showing ${activeFilter === 'All' ? 'the full catalog' : activeFilter + ' objects'}</span>
          </div>
