...
<body>
  ...
  <script>
    ...
    function topologyInteractionIcon(interaction) {
      const capability = interaction.capability || '';
      const name = String(interaction.name || '').toLowerCase();
      if (capability === 'load-balancing') return { icon: '🟢', cls: 'network' };
      if (name.includes('sftp')) return { icon: '📥', cls: 'storage' };
      if (capability === 'data-persistence') return { icon: '💾', cls: 'storage' };
      if (name.includes('lambda') || capability === 'compute') return { icon: 'λ', cls: 'compute' };
      if (name.includes('eks') || capability === 'container-orchestration') return { icon: '⬢', cls: 'compute' };
      return { icon: '☁', cls: 'cloud' };
    }
    ...
    function runsOnObjectForProductService(entry) {
      const service = objectLookup[entry.ref];
      return service?.runsOn ? objectLookup[service.runsOn] : null;
    }

    function inferServiceGroupTier(group) {
      const scores = { web: 0, app: 0, data: 0 };
      const addScore = tier => {
        if (scores[tier] !== undefined) scores[tier] += 1;
      };

      (group.productServices || []).forEach(entry => {
        const runsOn = runsOnObjectForProductService(entry);
        const category = runsOn?.serviceCategory || '';
        if (category === 'web') addScore('web');
        else if (category === 'dbms') addScore('data');
        else addScore('app');
      });

      (group.rbbs || []).forEach(entry => {
        const ref = String(entry.ref || '');
        const rbb = objectLookup[ref];
        const category = rbb?.serviceCategory || '';
        if (category === 'web' || ref.startsWith('rbb.service.web.')) addScore('web');
        else if (category === 'dbms' || ref.startsWith('rbb.service.dbms.')) addScore('data');
        else addScore('app');
      });

      (group.applianceAbbs || []).forEach(entry => {
        const appliance = objectLookup[entry.ref];
        const capability = appliance?.capability || '';
        if (capability === 'load-balancing') addScore('web');
        if (['file-storage', 'data-persistence', 'storage'].includes(capability)) addScore('data');
      });

      (group.saasServices || []).forEach(entry => {
        const saas = objectLookup[entry.ref];
        const capability = saas?.capability || '';
        if (['waf', 'identity'].includes(capability)) addScore('web');
        else if (['storage', 'analytics'].includes(capability)) addScore('data');
        else addScore('app');
      });

      const ranked = Object.entries(scores).sort((a, b) => b[1] - a[1]);
      return ranked[0][1] > 0 ? ranked[0][0] : 'app';
    }

    function tierColumnsMarkup(tierBuckets) {
      return `
        <div class="deployment-target-columns">
          <section class="topology-tier-column">
            <div class="topology-tier-header web">Web Services</div>
            ${tierBuckets.web.join('') || '<div class="empty-card">No web-tier service groups.</div>'}
          </section>
          <section class="topology-tier-column">
            <div class="topology-tier-header app">App Services</div>
            ${tierBuckets.app.join('') || '<div class="empty-card">No app-tier service groups.</div>'}
          </section>
          <section class="topology-tier-column">
            <div class="topology-tier-header data">Data Services</div>
            ${tierBuckets.data.join('') || '<div class="empty-card">No data-tier service groups.</div>'}
          </section>
        </div>
      `;
    }
    ...
    function renderDeploymentTopology(sdm) {
      ...
      const deploymentTargetMarkup = [...targets.entries()].map(([target, grouped]) => {
        ...
        const unscopedTierBuckets = { web: [], app: [], data: [] };
        const pushToTier = (bucket, tier, markup) => {
          const resolved = bucket[tier] ? tier : 'app';
          bucket[resolved].push(markup);
        };

        const scalingUnitMarkup = [...grouped.scoped.entries()].map(([scalingUnitName, groups]) => {
          const scalingTierBuckets = { web: [], app: [], data: [] };
          groups.forEach(group => {
            pushToTier(scalingTierBuckets, inferServiceGroupTier(group), serviceGroupTopologyMarkup(group));
          });
          return `
            <section class="scaling-unit-box">
              ...
              <div class="scaling-unit-content">
                ${tierColumnsMarkup(scalingTierBuckets)}
              </div>
            </section>
          `;
        }).join('');

        grouped.unscoped.forEach(group => {
          pushToTier(unscopedTierBuckets, inferServiceGroupTier(group), serviceGroupTopologyMarkup(group));
        });
        ...
        return `
          <section class="deployment-target-cluster">
            ...
            <div class="deployment-target-content">
              ${scalingUnitMarkup}
              ${grouped.unscoped.length ? tierColumnsMarkup(unscopedTierBuckets) : ''}
            </div>
          </section>
        `;
      }).join('');
