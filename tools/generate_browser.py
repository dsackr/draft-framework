  ...
  <script>
    ...
    function renderDeploymentTopology(sdm) {
      ...
      const deploymentTargetMarkup = [...targets.entries()].map(([target, grouped]) => {
        ...
        const tierBuckets = { web: [], app: [], data: [] };
        const pushToTier = (tier, markup) => {
          const resolved = tierBuckets[tier] ? tier : 'app';
          tierBuckets[resolved].push(markup);
        };

        [...grouped.scoped.entries()].forEach(([scalingUnitName, groups]) => {
          const firstTier = inferServiceGroupTier(groups[0] || {});
          pushToTier(firstTier, `
        ...
        grouped.unscoped.forEach(group => {
          pushToTier(inferServiceGroupTier(group), serviceGroupTopologyMarkup(group));
        });
