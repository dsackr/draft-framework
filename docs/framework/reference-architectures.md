...
## Why An RA Is Never A Node In A DA Diagram

An RA is not a deployed thing. It is a pattern declaration.

A DA may say `appliesPattern: ra.dotnet.three-tier.ha`, but that field is metadata about conformance, not a deployed runtime element. The visual question for a DA is “what RBBs are deployed here?” The governance question is “which RA pattern does this deployment claim to follow?”
...
### What is the difference between an RA and a DA?

An RA is generic and a DA is specific. An RA says what kinds of building blocks must exist for a pattern. A DA says which building blocks a specific product actually deploys.

### Can a product deviate from its RA?

Yes, but that should be treated as an explicit exception rather than invisible drift. If a product cannot follow the pattern, engineers should either document the exception clearly in the DA, propose a new RA, or revise the existing RA.
